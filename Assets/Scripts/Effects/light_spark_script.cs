...
public class light_spark_script : MonoBehaviour
{
    ...
    public Vector2 delayRandomRange; // add random to the delay
    ...
    private void Update()
    {
        ...
        if (myLight != null)
        {
            ...
            if (timeElapsed >= delay + randomRange)
            {
                // reset the time elapsed
                timeElapsed = 0;
                randomRange = Random.Range(delayRandomRange.x, delayRandomRange.y);
                // toggle the light
                ToggleLight();
            }
        }
